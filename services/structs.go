...
Эти функции проверяют поля, определенные структурой, и
пытаются разрешить их с помощью определенных служб. Любые поля,
тип которых не является интерфейсом или для которых нет службы,
пропускаются.
*/
